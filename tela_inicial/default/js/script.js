...
//
$('.link').click(function () {
	var $card = $(this).closest('li[data-item]');
	var $lista = $card.closest('ul[data-index]');
	var dataIndex = $lista.attr('data-index');


	if (dataIndex === 'lista-fazer') {
		var aux = parseInt($lista.attr('id'));
		var id = aux + 1;


		$card.removeClass('fazer feito').addClass('fazendo');
		$card.prependTo($("#" + id));
		...
		// Exibir ou ocultar elementos conforme necessário
		// if (localStorage.getItem(uc + '_verificaPrimeira') !== "nao_mais") {
		//  $('.setinha, .exemplo').show();
		// } else {
		//  $('.setinha, .exemplo').hide();
		// }
	}
});
